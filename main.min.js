...
const algorithm = (pizzas, x, y, z) => {
    let data_array = [];
    total = Math.floor(2 * x + 3 * y + 4 * z);
    for (let index = 0; index < x; index++) data_array.push(2);
    for (let index = 0; index < y; index++) data_array.push(3);
    for (let index = 0; index < z; index++) data_array.push(4);
    let k = [];
    for (let index = 0; index < data_array.length; index++) {
        let a = data_array[index],
            o;
        k.push(a), 2 == k.length && (o = k[0] + k[1])
    }
    return data_array = data_array, data_array
};
algorithm(d[0], d[1], d[2], d[3]);
let cf = [];
const rep = element => {
    let gv = [];
    gv.push(element), cf.push(gv)
};
let rt = [];
const gt = () => {
...
class bbg {
    constructor(elm, index) {
        this.element = elm, this.index = index, i++;
        for (let index = 0; index < cf.length - i; index++) {
            let g;
            g = cf[index + i].map(arg => [elm, arg]), rt.push(g)
        }
    }
}
gt();
...
const available_teams = pizza => {
    rt.forEach((root, i) => {
        root.forEach(element => {
            if (element[0] + element[1] == 5) return teams_to_get_deliveries.push(root), teams_to_get_deliveries
        })
    })
};
available_teams();
const maximizing_the_ingredients = pizza => {
    ...
    }), total_ingredients.total += total_ingredients.ingredients.length, total_ingredients.unique_ingredients = [...new Set(total_ingredients.ingredients)], console.log(total_ingredients)
};
